...
class Seq2Seq(nn.Module):
    def __init__(self, bert_model, output_size, embedding_dim):
        ...
        self.encoder.eval()
        self.encoder_GRU = nn.GRU(768 ,512 ,batch_first=True)

        self.decoder_embedding = nn.Embedding(output_size, embedding_dim)
        self.decoder = nn.GRU(embedding_dim, 512, batch_first=True)
    ...
    def forward(self, input1, input2):
        ...
        encoder_GRU_out,encoder_GRU_hidden = self.encoder_GRU(encoder_hidden_state)
        # print('encoder_GRU_hidden',encoder_GRU_hidden.size())
        # print('encoder_GRU_out',encoder_GRU_out.size())
        decoder_embedded = self.decoder_embedding(input2)
        decoder_output, _ = self.decoder(decoder_embedded,
                                         encoder_GRU_hidden)#給最後的hidden state
