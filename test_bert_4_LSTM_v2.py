...
def test(model, bert_tokenizer,input_text,index_word,word_index,max_len):
    ...
    with torch.no_grad():
        inputs1 = input_text
        input_text_input_ids = input_text['input_ids'][0].to(device)
        input_text_token_type_ids = input_text['token_type_ids'][0].to(device)
        input_text_attention_mask = input_text['attention_mask'][0].to(device)
...
def test2(model, bert_tokenizer,input_text,index_word,word_index,max_len):
    ...
    with torch.no_grad():
        ...
        for num_word in range(MAX_LEN):
            ...
            if num_word == 0:
                top_3 = torch.topk(outputs[:, num_word-1, :].flatten(), 3).indices
                random_select = np.random.randint(0,3)
                random_select_top3 = top_3[random_select]
                outputs_index = random_select_top3.detach().cpu().numpy()
            else:
                outputs_argmax = torch.argmax(outputs,dim=-1)
                outputs_index = outputs_argmax[0][num_word].detach().cpu().numpy()
...
if __name__ =="__main__":
    ...
    WEIGHTS = f'{CURRENT_PATH}/weights/model_bert_4_LSTM_emd200_v2/{WEIGHTS_NAME}'
